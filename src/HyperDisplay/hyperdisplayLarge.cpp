...
#include  "hyperdisplayLarge.h" 					// Click here to get the library: http://librarymanager/SparkFun_HyperDisplay
// #include "arduino.h"
...
void        hyperdisplay::show( wind_info_t * wind ){   // Outputs the current window's buffered data to the display
    ...
    // Serial.println("Show:");
    // Serial.println(pCurrentWindow->xMax - pCurrentWindow->xMin);
    // Serial.println(pCurrentWindow->yMax - pCurrentWindow->yMin);
    // Serial.println((unsigned int)pCurrentWindow->data);
    // Serial.println(pCurrentWindow->numPixels);
	...
	size_t hyperdisplay::write(uint8_t val)
	{
      ...
      // ORIG Size
      	...
      	// Figure out if the character should cause a newline
			...
			uint16_t offset = 6 + 5 * (character - 0); // ORIG Size
			// uint16_t offset = font10x14[3] + 5 * (character - 0);
			character_info->numPixels = 0;
			uint16_t n = 0;
			for(uint8_t indi = 0; indi < 5 ; indi++)
			{
				values[indi] = pgm_read_byte(font5x7 + offset + indi);
				// values[indi] = pgm_read_byte(font10x14 + offset + indi);  // ORIG Size
				for(uint8_t indj = 0; indj < 8; indj++)
				{
					if(values[indi] & (0x01 << indj))
					{
						character_info->numPixels++;
						*(character_info->xLoc + n) = (hd_font_extent_t)(2*indi);
						*(character_info->yLoc + n) = (hd_font_extent_t)(2*indj);
						n++;

						character_info->numPixels++;
						*(character_info->xLoc + n) = (hd_font_extent_t)(2*indi+1);
						*(character_info->yLoc + n) = (hd_font_extent_t)(2*indj);
						n++;

						character_info->numPixels++;
						*(character_info->xLoc + n) = (hd_font_extent_t)(2*indi);
						*(character_info->yLoc + n) = (hd_font_extent_t)(2*indj+1);
						n++;
